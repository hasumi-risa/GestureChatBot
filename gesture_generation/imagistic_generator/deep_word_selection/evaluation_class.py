...
def evaluate(model, test_loader, interval=1):
    ...
    with torch.no_grad():
        for text, labels in tqdm(test_loader):
            ...
            output = model(text)
            predicted = output.reshape(output.shape[0], -1)
            ...
            for i in range(len(predicted)):
                token = tokenizer.convert_ids_to_tokens(text[i][:text_len[i]])

                pred = predicted[i][:text_len[i]].to('cpu').detach().numpy().copy()
                ...
                gt = labels[i][:text_len[i]].reshape(-1, 1).to('cpu').detach().numpy().copy()
                value = np.hstack([gt, pred])

                filename = re.sub(r'[\\/:*?"<>|]+','',"_".join(token[1:])[0:30])
                fig, ax = plt.subplots()
                sns.heatmap(data=value, cmap='OrRd', annot=True, vmin=0, vmax=1)
                ax.set_xticklabels(['GT', 'Predicted'], rotation=0)
                ax.set_yticks(np.arange(text_len[i]))
                ax.set_yticklabels(token, rotation=0)
                plt.tight_layout()
                plt.savefig("./imgs/test_train_norm/{}.png".format(filename))
                # plt.show()
...
if __name__ == '__main__':
    ...
    for model_mode in model_modes:
        ...
        data_dir = './data/'
        model_path = './model_data/model_{}_{}_{}.pt'.format(model_kind, option1, model_mode) 
        batch_size = 4
        MAX_SEQ_LEN = 256
        ...
        if data_kind == 'test':
            data_iter = createDataLoader(data_dir + 'test_{}.pth'.format(option1))
        elif data_kind == 'train':
            data_iter = createDataLoader(data_dir + 'train_{}.pth'.format(option1))
        ...
        # Save Images
        save_dir = './images/prediction_class/{}_{}_{}_{}/'.format(data_kind, model_mode, option1, model_kind)
