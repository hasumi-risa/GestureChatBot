...
class Plot():
    ...
    def animate_frame(self, pose):
        if len(pose) == 25:
            ...
            self.line6.set_data([pose[10], pose[12]], [pose[11], pose[13]])
            self.line7.set_data([pose[12], pose[14]], [pose[13], pose[15]])
            if pose[16] == "No-Gesture":
                plt.title('{}'.format(pose[16]), color='g')
            elif pose[16] == "Beat":
                plt.title('{}'.format(pose[16]), color='b')
            elif pose[16] == "Imagistic":
                plt.title('{}'.format(pose[16]), color='r')
            else:
                plt.title('{}'.format(pose[16]), color='k')
