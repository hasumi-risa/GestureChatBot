...
import gui.messagebox as messagebox
...
import kinect
import labanotation.alglowerbody as alglowerbody
...
class application:
    ...
    def __init__(self):
        ...
        if (self.fShowGUI):
            ...
            self.graphFilterLB = graphFilter.graphFilter()

            # create a graph for the labanotation visualization
            self.graphLaban = graphLaban.graphLaban()

            # create a graph for the lower body data
            self.graphLowerBody = alglowerbody.GraphLowerBody()
    ...
    def determineFilePaths(self):
        ...
        if os.path.isabs(self.inputName):
            ...
            self.inputFilePath = os.path.join(settings.cwd, self.inputName)
    ...
    def selectTime(self, time, caller=None):
        ...
        if (self.graphLowerBody is not None):
            self.graphLowerBody.selectTime(time)

    #------------------------------------------------------------------------------
    #
    def applyAlgorithm(self, algorithm = None, forceReset = False):
        if algorithm is not None:
            self.algorithm = algorithm.lower()

        if self.algorithm is None:
            self.logMessage("Can't apply algorithm--none has been set")
            return

        self.logMessage("Applying algorihm '" + self.algorithm + "'...")

        ax = self.graphFilter.ax if (self.graphFilter != None) else None
        axLB = self.graphFilterLB.ax if (self.graphFilterLB != None) else None

        [self.timeS, self.all_laban] = self.labanotation.applyAlgorithm(ax, axLB, self.graphLowerBody, self.jointFrames, self.algorithm, forceReset)
        ...
        if (self.graphFilterLB != None):
            self.graphFilterLB.fig.canvas.draw_idle()

        if (self.graphLowerBody != None):
            self.graphLowerBody.fig.canvas.draw_idle()
    ...
    def openAndProcessInputfile(self, file = None):
        ...
        if (file is not None):
            if (os.path.isabs(file)):
                ...
                messagebox.showerror("Labanotation", "The file '" + str(file) + "' is not a valid file.", centeronfig=self.guiMenu.fig)
                return
        ...
        if (self.graphLowerBody != None):
            self.graphLowerBody.updateInputName()

        # apply selected algorithm
        self.applyAlgorithm(self.algorithm, forceReset)
