...
class graphLaban:
    ...
    def onMotion(self, event):
        ...
        if not self.drag_active:
            # Enter drag mode if mouse movement exceeds dead-zone threshold
            if abs(event.y - self.y0) > 4:
    ...
    def onRelease(self, event):
        ...
        self.press = None
        if self.drag_active:
            # User dragged the view
            self.drag_active = False
        else:
            # Didn't enter drag mode, get the time at location clicked
            proportionTime = (self.view.timeOffset - self.ypress) / self.view.timeScale
            if (proportionTime < 0.0):
                proportionTime = 0.0
            elif (proportionTime > 1.0):
                proportionTime = 1.0

            # Select the time but temporarily disable auto-centering so the view doesn't jump around
            center_on_selected_time_sav = self.center_on_selected_time
            self.center_on_selected_time = False

            settings.application.selectTime(proportionTime)

            self.center_on_selected_time = center_on_selected_time_sav
    ...
    def setLabanotation(self, timeS, all_laban):
        ...
        self.cursor = Cursor(self.ax, color='red', linewidth=0.5, vertOn=False, useblit=True)
    ...
    def selectTime(self, time):
        ...
        if self.center_on_selected_time:
